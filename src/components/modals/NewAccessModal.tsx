...
import { ArrowLeft, MapPin, Route, X } from 'lucide-react';
import type { Beach } from '../../types';
import { AccessRegistrationGuidePanel } from '../guide/AccessRegistrationGuidePanel';

export type AccessFormStep = 'map' | 'details';
...
interface NewAccessModalProps {
  ...
  setNewAccessBlocker: (blocker: NewAccessModalProps['newAccessBlocker']) => void;
  ...
  setNewAccessAmenities: (amenities: NewAccessModalProps['newAccessAmenities']) => void;
  newAccessAccessibility: {
    ramps: boolean;
    wheelchair: boolean;
    parkingReserved: boolean;
  };
  setNewAccessAccessibility: (accessibility: NewAccessModalProps['newAccessAccessibility']) => void;
  onStartMapWork: () => void;
  onBackToMap: () => void;
      ...
      >
        <div className="flex items-center justify-between pb-3 border-b border-gray-250 mb-4">
          <div className="text-left">
            <h3 className="text-base font-bold text-gray-905">
              {isMapStep ? 'Paso 1: Ubicar acceso en el mapa' : 'Paso 2: Datos del acceso'}
            </h3>
            <p className="text-[9.5px] text-gray-500 mt-0.5">
              {isMapStep
                ? 'Fije la entrada y, si corresponde, trace el sendero antes de completar la ficha'
        ...
        {isMapStep ? (
          <div className="space-y-5 text-left">
            ...
            <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 text-[10.5px] text-blue-900">
              <p>
                <strong>Orden del registro:</strong> primero entrada en el mapa, sendero opcional y
                después ficha de datos. La playa vinculada puede detectarse automáticamente según la
            ...
            <button
              type="button"
              onClick={onStartMapWork}
              disabled={beaches.length === 0}
              className="w-full py-3.5 flex items-center justify-center gap-2 bg-[#0871E7] hover:brightness-105 rounded-xl font-semibold text-white transition-all shadow disabled:opacity-50"
            >
              <MapPin size={16} />
              <span>Comenzar registro en el mapa</span>
            </button>
          </div>
        ) : (
          <form onSubmit={onSubmit} className="space-y-4 text-xs text-left">
            <div className="p-3 bg-blue-50 rounded-xl border border-blue-200 text-blue-900 text-[10.5px] flex items-start justify-between gap-3">
              <div className="space-y-1">
                <p>
                  <MapPin size={11} className="inline mr-1" />
                  Entrada fijada en el mapa
                </p>
                <p>
                  ...
                  <strong>
                    {trailDrawingPoints.length > 1
                      ? `${trailDrawingPoints.length} vértices`
                      : 'No registrado'}
                  </strong>
                </p>
              </div>
              <button
                type="button"
                onClick={onBackToMap}
                className="shrink-0 inline-flex items-center gap-1 text-blue-700 hover:text-blue-900 font-semibold underline"
              >
                <ArrowLeft size={12} />
                Editar en mapa
              </button>
            ...
            {placedPinCoordinates && nearestBeach && (
              <div className="p-3.5 rounded-xl bg-blue-50 border border-blue-200 text-blue-900 text-[10.5px] leading-relaxed">
                {nearestBeach.distance < 3.0 ? (
                  <span>
                    Playa vinculada automáticamente: <strong>{nearestBeach.beach.name}</strong> a
                    una distancia estimada de{' '}
                    <strong>{(nearestBeach.distance * 1000).toFixed(0)} metros</strong> de la
                    entrada marcada.
                  </span>
                ) : (
                  <span className="text-amber-800">
                    <strong>Advertencia:</strong> La playa más cercana está a{' '}
                    <strong>{nearestBeach.distance.toFixed(1)} km</strong> de este acceso. Verifique
                    la vinculación manualmente.
                  </span>
                )}
              </div>
            )}
            ...
            <div>
              <label className="block text-[9px] font-bold uppercase text-gray-600 mb-1.5">
                Servicios de acceso y a11y
              </label>
              <div className="grid grid-cols-2 gap-x-4 gap-y-2 p-3 rounded-xl bg-gray-205/50 border border-gray-300/40 text-gray-700">
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setNewAccessAmenities({ ...newAccessAmenities, parking: e.target.checked })
                    }
                ...
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setNewAccessAmenities({ ...newAccessAmenities, security: e.target.checked })
                    }
                    className="accent-[#0871E7]"
                  />
                ...
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setNewAccessAmenities({ ...newAccessAmenities, showers: e.target.checked })
                    }
                    className="accent-[#0871E7]"
                  />
                ...
                <label className="flex items-center gap-1.5 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setNewAccessAmenities({ ...newAccessAmenities, pets: e.target.checked })
                    }
                    className="accent-[#0871E7]"
                  />
            ...
            <div>
              <label className="block text-[9px] font-bold uppercase text-gray-600 mb-1">
                Estado de obstrucción de acceso
              </label>
              <select
                value={newAccessBlocker}
                onChange={(e) => setNewAccessBlocker(e.target.value as NewAccessModalProps['newAccessBlocker'])}
            ...
            {newAccessBlocker !== 'None' && (
              <div className="space-y-3 p-3.5 rounded-xl bg-red-50 border border-red-200/60 transition-all duration-300">
                <div>
                  <label className="block text-[9px] font-bold uppercase text-red-705 mb-1">
            ...
            <button
              type="submit"
              disabled={!placedPinCoordinates || !newAccessBeachId}
              className="w-full py-3.5 bg-[#0871E7] hover:brightness-105 rounded-xl font-semibold text-white transition-all shadow disabled:opacity-50"
            >
              Guardar acceso registrado
            </button>
          </form>
        )}
      </div>
